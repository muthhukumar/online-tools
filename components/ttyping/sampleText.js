...
const SampleText = ({ data }) => {
  ...
  const submitHandler = e => {
    ...
    try {
      ...
      db.collection('warehouse').add({
        crop: `${lettersCount} letters ${wordsCount}words ${wps}wps ${minutes}min mode:${levelRef.current.value}`,
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (buttonText !== 'save') setButtonText('saved');
    }, 5000);
    ...
    <div className="w-11/12 mx-auto rounded-sm shadow-lg flex-col rounded-lg">
      ...
      <div className="w-full flex flex-wrap p-2 border border-gray-300 mx-auto">
        {targetWords.map(text => (
          <Text content={text.content} key={text.id} color={text.color} />
        ))}
      </div>
      ...
      <div className="flex items-center py-2 justify-between w-full bg-gray-100 text-gray-700">
        ...
        <div className="flex items-center">
          <select
            ref={levelRef}
            type="dropdown"
            className="px-1 border border-lg border-teal-600 focus:outline-none mx-2 py-2 text-teal-600 bg-gray-100"
          >
          ...
          <button
            ...
            className="px-4 py-2 border rounded-sm mr-2 border-green-400 text-green-400 focus:outline-none hover:border-green-700 hover:text-green-700"
          >
            {buttonText}
          </button>
        </div>
